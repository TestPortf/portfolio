...
def main():
    ...
    from scipy.signal import daub, cwt
    from pywt import dwt, Wavelet, wavedec, downcoef

    # Формируем полезный сигнал
    signal_discretization = 2**(-7)
    signal_alpha = 2.5
    useful_signal_length = 2**7
    useful_singal_range = np.linspace(0, useful_signal_length - 1, useful_signal_length)
    useful_signal = 10 * useful_singal_range * signal_discretization * exp(-signal_alpha * useful_singal_range * signal_discretization)

    main_signal_length = 2**5
    main_signal_time = np.arange(0, 2, signal_discretization)
    main_signal_range = np.linspace(0, main_signal_length - 1, main_signal_length)
    shift_size = 360 
    main_signal = []
    ...
    harmonic_part = sin(2 * pi * (main_signal_range * signal_discretization)**2) + cos(3 * pi * (main_signal_range * signal_discretization)**2)
    additive_harm_signal = harmonic_part + main_signal

    # Generate gaussian noise
    noise_dispersion = 1
    gaussian_noise = np.random.normal(0, noise_dispersion, main_signal_length)
    gaussian_noise_signal = additive_harm_signal + gaussian_noise
