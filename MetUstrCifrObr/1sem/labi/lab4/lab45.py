...
def signal_plot(input_signal):
    ...
    k = np.linspace(0, int(len(input_signal)/2), int(len(input_signal)/2))
    frequency = k/(int(len(input_signal)) * delta_T)
    print(len(k), len(frequency))
    
    # Строим сигнал
    plt.figure()
    plt.xlabel('Отсчёты')
    plt.ylabel('Амплитуда')
    plt.plot(frequency, input_signal[:int(len(input_signal)/2)])
...
def main():
    ...
    signal_plot(abs(fft(bit0)))
    signal_plot(abs(fft(bit1)))

    # Генереируем фильтр 
    discrete_analog_filter = generate_filter('BPF', 'Bessel', 8, fv=2.5, fn=1)
